    ...
    return (
        ...
        >
            <View className='flex-1 px-4'>
                <Text className="text-lg text-zinc-800 dark:text-zinc-300 font-semibold mb-4 text-center">Change Theme</Text>
                {THEMES.map((theme) => (
                    <TouchableOpacity
                        key={theme.id}
                        className={`flex-row items-center px-4 py-3 rounded-xl mb-2 ${selected === theme.id ? (theme.id === "1" ? "bg-theme1-primary/20 dark:bg-theme1-primary/20" : "bg-theme2-primary/20 dark:bg-theme2-primary/20") : 'bg-zinc-200 dark:bg-zinc-900/60'}`}
                        activeOpacity={0.8}

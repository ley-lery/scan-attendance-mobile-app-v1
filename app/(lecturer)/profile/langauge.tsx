    ...
    return (
        ...
        >
            <View className='flex-1 px-4'>
                <Text className="text-lg text-white font-semibold mb-4 text-center">Change Language</Text>
                {LANGUAGES.map((lang) => (
                    <TouchableOpacity
                        key={lang.code}
                        className={`flex-row items-center px-4 py-3 rounded-xl mb-2 ${selected === lang.code ? 'bg-pink-100 dark:bg-pink-900/30' : 'bg-zinc-900/60'}`}
                        activeOpacity={0.8}
